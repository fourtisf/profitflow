// validate.js — run locally with your own Helius key.
//   HELIUS_API_KEY=xxx node validate.js <WALLET> <TOKEN_MINT> [SOL_USD]
//
...
// Compare the printed ledger line-by-line against Solscan to verify accuracy.
//
...
if (!KEY || !WALLET || !MINT) {
  console.error('Usage: HELIUS_API_KEY=xxx node validate.js <WALLET> <TOKEN_MINT> [SOL_USD]');
  console.error('  SOL_USD optional flat override. For real accuracy use per-timestamp pricing (see NOTE below).');
  process.exit(1);
}

// --- SOL -> USD --------------------------------------------------------------
// NOTE: a single flat price is WRONG for historical accuracy. Production must price each
...
function solToUsd(_ts, sol) { return sol * FLAT_SOL; }

async function fetchAll(addr) {
  const out = [];
  let before = '';
  for (let page = 0; page < 50; page++) {                      // hard cap 5000 tx
...
function valueDelta(tx, wallet, ts) {
  let usd = 0;
  // native SOL
...
function classify(tx, mint, wallet) {
...
(async () => {
  ...
  if (!events.length) { console.log('No activity for this token in this wallet.'); return; }
  ...
  for (const e of r.ledger) {
    ...
    console.log(`  ${dt(e.ts)}  ${tag}${amt}${usd}  ${e.sig.slice(0, 8)}…${rz}`);
  }
  ...
  console.log(`  Still holding       ${r.tokensRemaining.toLocaleString()} tokens (cost basis $${r.costBasisRemaining.toLocaleString()})`);
